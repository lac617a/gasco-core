...
import { configureDismissInteraction, configureKeyboardInteraction, configureTriggerInteraction } from './gasco-popover-utils';
...
@Component({
  tag: 'gasco-popover',
  styleUrl: 'gasco-popover.md.scss',
  shadow: true,
})
export class GascoPopover implements ComponentInterface, PopoverInterface {
  private usersElement?: HTMLElement;
  private triggerEl?: HTMLElement | null;
  private parentPopover: HTMLGascoPopoverElement | null = null;
  ...
  @Element() el!: HTMLGascoPopoverElement;
   ...
   * navigating `gasco-list` items within a popover (up/down/home/end/etc.).
   ...
   */
  @Event({ eventName: 'gascoPopoverDidPresent' }) didPresent!: EventEmitter<void>;

  /**
   * Emitted before the popover has presented.
   */
  @Event({ eventName: 'gascoPopoverWillPresent' }) willPresent!: EventEmitter<void>;

  /**
   * Emitted before the popover has dismissed.
   */
  @Event({ eventName: 'gascoPopoverWillDismiss' }) willDismiss!: EventEmitter<OverlayEventDetail>;

  /**
   * Emitted after the popover has dismissed.
   */
  @Event({ eventName: 'gascoPopoverDidDismiss' }) didDismiss!: EventEmitter<OverlayEventDetail>;
  ...
  componentWillLoad() {
     ...
     */
    this.popoverId = this.el.hasAttribute('id') ? this.el.getAttribute('id')! : `gasco-popover-${this.popoverIndex}`;

    this.parentPopover = this.el.closest(`gasco-popover:not(#${this.popoverId})`) as HTMLGascoPopoverElement | null;
  ...
  componentDidLoad() {
    ...
    if (parentPopover) {
      addEventListener(parentPopover, 'gascoPopoverWillDismiss', () => {
  ...
  async present(event?: MouseEvent | TouchEvent | PointerEvent | CustomEvent): Promise<void> {
    ...
    this.usersElement = await attachComponent(delegate, this.el, this.component, data, inline, ['popover-viewport']);
    ...
    this.currentTransition = present(this, 'popoverEnter', mdEnterAnimation, {
  ...
  async dismiss(data?: any, role?: string, dismissParentPopover = true): Promise<boolean> {
    ...
    this.currentTransition = dismiss(this, data, role, 'popoverLeave', mdLeaveAnimation, this.event);
   ...
   */
  @Method()
  async getParentPopover(): Promise<HTMLGascoPopoverElement | null> {
  ...
  onDidDismiss<T = any>(): Promise<OverlayEventDetail<T>> {
    return eventMethod(this.el, 'gascoPopoverDidDismiss');
  }
  ...
  onWillDismiss<T = any>(): Promise<OverlayEventDetail<T>> {
    return eventMethod(this.el, 'gascoPopoverWillDismiss');
  }
  ...
  render() {
    ...
    return (
      ...
      >
        {!parentPopover && <gasco-backdrop tappable={this.backdropDismiss} visible={this.showBackdrop} part="backdrop" />}

        <div class="popover-wrapper gasco-overlay-wrapper" onClick={dismissOnSelect ? () => this.dismiss() : undefined}>
...
const LIFECYCLE_MAP: any = {
  gascoPopoverDidPresent: 'gascoViewDidEnter',
  gascoPopoverWillPresent: 'gascoViewWillEnter',
  gascoPopoverWillDismiss: 'gascoViewWillLeave',
  gascoPopoverDidDismiss: 'gascoViewDidLeave',
};
