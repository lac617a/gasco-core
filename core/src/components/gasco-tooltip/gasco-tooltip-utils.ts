...
export const configureDismissInteraction = (
  ...
  popoverEl: HTMLGascoTooltipElement,
  parentPopoverEl: HTMLGascoTooltipElement
) => {
      ...
      dismissCallbacks = [
        {
          ...
          callback: (ev: MouseEvent) => {
            ...
            const closestTrigger = target.closest('[data-gasco-tooltip-trigger]');
...
export const configureTriggerInteraction = (
      ...
      triggerCallbacks = [
        ...
        },
        {
          ...
          callback: (ev: MouseEvent) => {
            ...
            if (target.closest('gasco-tooltip') !== popoverEl) {
              popoverEl.dismiss(undefined, undefined, false);
            }
          },
        },
        ...
        {
          eventName: 'gascoTooltipActivateTrigger',
  ...
  triggerEl.setAttribute('data-gasco-tooltip-trigger', 'true');

  return () => {
    triggerCallbacks.forEach(({ eventName, callback }) => triggerEl.removeEventListener(eventName, callback));
    triggerEl.removeAttribute('data-gasco-tooltip-trigger');
  };
};

/**
 * Returns the index of an gasco-item in an array of gasco-items.
 */
export const getIndexOfItem = (items: HTMLGascoItemElement[], item: HTMLElement | null) => {
  if (!item || item.tagName !== 'GASCO-ITEM') {
    return -1;
  }
...
/**
 * Given an array of elements and a currently focused gasco-item
 * returns the next gasco-item relative to the focused one or
 * undefined.
 */
export const getNextItem = (items: HTMLGascoItemElement[], currentItem: HTMLElement | null) => {
...
/**
 * Given an array of elements and a currently focused gasco-item
 * returns the previous gasco-item relative to the focused one or
 * undefined.
 */
export const getPrevItem = (items: HTMLGascoItemElement[], currentItem: HTMLElement | null) => {
  const currentItemIndex = getIndexOfItem(items, currentItem);
  return items[currentItemIndex - 1];
};

/** Focus the internal button of the gasco-item */
const focusItem = (item: HTMLGascoItemElement) => {
 ...
 * as a trigger element for an gasco-popover.
 */
export const isTriggerElement = (el: HTMLElement) => el.hasAttribute('data-gasco-tooltip-trigger');

export const configureKeyboardInteraction = (popoverEl: HTMLGascoTooltipElement) => {
  const callback = async (ev: KeyboardEvent) => {
    const activeElement = document.activeElement as HTMLElement | null;
    let items: HTMLGascoItemElement[] = [];

    const targetTagName = (ev.target as HTMLElement)?.tagName;
    /**
     * Only handle custom keyboard interactions for the host popover element
     * and children gasco-item elements.
     */
    if (targetTagName !== 'GASCO-TOOLTIP' && targetTagName !== 'GASCO-ITEM') {
      return;
    }
    /**
    ...
    try {
      /**
       * Select all gasco-items that are not children of child popovers.
       * i.e. only select gasco-item elements that are part of this popover
       */
      items = Array.from(
        popoverEl.querySelectorAll(
          'gasco-item:not(gasco-popover gasco-popover *):not([disabled])'
        ) as NodeListOf<HTMLGascoItemElement>
      );
    ...
    switch (ev.key) {
      ...
      case 'ArrowLeft':
        ...
        break;
      /**
       * ArrowDown should move focus to the next focusable gasco-item.
       */
      case 'ArrowDown':
        ...
        break;
      /**
       * ArrowUp should move focus to the previous focusable gasco-item.
       */
      case 'ArrowUp':
        // Disable movement/scroll with keyboard
        ev.preventDefault();
        ...
        break;
      /**
       * Home should move focus to the first focusable gasco-item.
       */
      case 'Home':
        ev.preventDefault();
        ...
        break;
      /**
       * End should move focus to the last focusable gasco-item.
       */
      case 'End':
        ev.preventDefault();
      ...
      case 'Enter':
        if (activeElement && isTriggerElement(activeElement)) {
          const rightEvent = new CustomEvent('gascoTooltipActivateTrigger');
...
export const getPopoverPosition = (
  ...
  switch (reference) {
    ...
    default:
       ...
       * of `gasco-breadcrumb` and centering it relative
       * to the indicator rather than `gasco-breadcrumb`
       * as a whole.
       */
