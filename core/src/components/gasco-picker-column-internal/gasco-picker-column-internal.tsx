...
@Component({
  ...
  styleUrl: 'picker-column-internal.md.scss',
  shadow: true,
})
export class GascoPickerColumnInternal implements ComponentInterface {
  ...
  @Element() el!: HTMLGascoPickerColumnInternalElement;
   ...
   */
  @Event() gascoChange!: EventEmitter<PickerColumnItem>;
  ...
  componentWillLoad() {
    ...
    const parentEl = this.el.closest('gasco-picker-internal') as HTMLGascoPickerInternalElement | null;
    if (parentEl !== null) {
      parentEl.addEventListener('gascoInputModeChange', (ev: any) => this.inputModeChange(ev));
    }
  }
  ...
  private setValue(value?: string | number) {
    ...
    if (findItem) {
      this.gascoChange.emit(findItem);
    }
  }
  ...
  /**
   * When gascoInputModeChange is emitted, each column
   ...
   * in the view and will emit an gascoChange with
   * the item object.
   */
