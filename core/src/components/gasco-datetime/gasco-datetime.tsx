...
@Component({
  tag: 'gasco-datetime',
  styleUrl: 'gasco-datetime.md.scss',
  shadow: true,
})
export class GascoDatetime implements ComponentInterface {
  ...
  @Element() el!: HTMLGascoDatetimeElement;
  ...
  /**
   * The locale to use for `gasco-datetime`. This
  ...
  /**
   * The first day of the week to use for `gasco-datetime`. The
  ...
  protected valueChanged() {
    if (this.hasValue()) {
      ...
      if (valueDateParts) {
        ...
        printGascoWarning(`Unable to parse date string: ${this.value}. Please provide a valid ISO 8601 datetime string.`);
      }
    }
  ...
  /**
   * The hour cycle of the `gasco-datetime`. If no value is set, this is
   * specified by the current locale.
   */
  @Prop() hourCycle?: 'h23' | 'h12';

  /**
   * If `cover`, the `gasco-datetime` will expand to cover the full width of its container.
  ...
  private initializeKeyboardListeners = () => {
    ...
    const checkCalendarBodyFocus = (ev: MutationRecord[]) => {
       ...
       * the inner day.
       */
      if (record.oldValue?.includes('gasco-focused') || !calendarBodyRef.classList.contains('gasco-focused')) {
        return;
      }
  ...
  componentDidLoad() {
    ...
    /**
     * Datetime uses Gasco components that emit
     * gascoFocus and gascoBlur. These events are
     * composed meaning they will cross
     * the shadow dom boundary. We need to
     * stop propagation on these events otherwise
     * developers will see 2 gascoFocus or 2 gascoBlur
     * events at a time.
     */
    const root = getElementRoot(this.el);
    root.addEventListener('gascoFocus', (ev: Event) => ev.stopPropagation());
    root.addEventListener('gascoBlur', (ev: Event) => {
      ev.stopPropagation()
    });
  }
      ...
      <div class="calendar-header">
        <div class="calendar-action-buttons">
          ...
          <div class="calendar-next-prev">
            <gasco-button-icon shape="round" fill="clear" size="small" disabled={prevMonthDisabled} onClick={() => this.prevMonth()}>
              <ion-icon icon={chevronBack} lazy={false} flipRtl></ion-icon>
            </gasco-button-icon>
            <gasco-button-icon shape="round" fill="clear" size="small" disabled={nextMonthDisabled} onClick={() => this.nextMonth()}>
  ...
  private renderCalendarBody() {
    return (
      <div class="calendar-body gasco-focusable" ref={(el) => (this.calendarBodyRef = el)} tabindex="0">
    ...
    return [
      ...
      <button
        ...
        onClick={async (ev) => {
          ...
          if (popoverRef) {
            ...
            popoverRef.present(
              new CustomEvent('gascoShadowTarget', {
                detail: {
                  gascoShadowTarget: ev.target,
                },
              })
            );
      ...
      <gasco-popover
        ...
        onWillPresent={(ev) => {
           ...
           */
          const cols = (ev.target! as HTMLElement).querySelectorAll('gasco-picker-column-internal');
  ...
  private renderCalendarViewHeader() {
    ...
    return (
      <div class="datetime-header">
        ...
        <div class="datetime-selected-date">{getMonthAndDay(this.locale, this.activeParts)}</div>
      </div>
    );
  }
