...
@Component({
  tag: 'gasco-content',
  styleUrl: 'gasco-content.scss',
  shadow: true,
})
export class GascoContent implements ComponentInterface {
  ...
  @Element() el!: HTMLGascoContentElement;
  ...
  /**
   * Because of performance reasons, gascoScroll events are disabled by default, in order to enable them
   ...
   */
  @Event() gascoScrollStart!: EventEmitter<ScrollBaseDetail>;

  /**
   * Emitted while scrolling. This event is disabled by default.
   * Set `scrollEvents` to `true` to enable.
   */
  @Event() gascoScroll!: EventEmitter<ScrollDetail>;

  /**
   * Emitted when the scroll has ended. This event is disabled by default.
   * Set `scrollEvents` to `true` to enable.
   */
  @Event() gascoScrollEnd!: EventEmitter<ScrollBaseDetail>;

  connectedCallback() {
    this.isMainContent = this.el.closest('gasco-menu, gasco-popover, gasco-modal') === null;
  }
  ...
  private onScroll(ev: UIEvent) {
    ...
    if (!this.queued && this.scrollEvents) {
      ...
      readTask((ts) => {
        ...
        this.gascoScroll.emit(this.detail);
      });
    }
  }
    ...
    return (
      <Host
        class={createColorClasses(this.color, {
          'content-sizing': hostContext('gasco-popover', this.el),
          [`content-${rtl}`]: true,
        })}
        style={{
...
const getPageElement = (el: HTMLElement) => {
  const tabs = el.closest('gasco-tabs');
  if (tabs) {
    return tabs;
  }
   ...
   */
  const page = el.closest('gasco-app, gasco-page, .gasco-page, page-inner, .popover-content');
  if (page) {
    return page;
  }
