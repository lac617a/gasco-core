...
export const createSwipeToCloseGesture = (el: HTMLGascoModalElement, animation: Animation, onDismiss: () => void) => {
  /**
  ...
  const getScrollY = () => {
    if (contentEl) {
      return (contentEl as HTMLGascoContentElement).scrollY;
      /**
  ...
  const canStart = (detail: GestureDetail) => {
     ...
     * with scrolling.
     *
     * We cannot assume that the `gasco-content`
     * target will remain consistent between
     * swipes. For example, when using
     * gasco-nav within a card modal it is
    ...
    if (contentEl) {
      ...
      const hasRefresherInContent = !!contentEl.querySelector('gasco-refresher');
     ...
     */
    const footer = target.closest('gasco-footer');
    if (footer === null) {
      return true;
    }
