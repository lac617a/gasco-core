...
const createMenuController = () => {
  ...
  const enable = async (shouldEnable: boolean, menu?: string | null): Promise<HTMLGascoMenuElement | undefined> => {
    const menuEl = await get(menu);
    if (menuEl) {
  ...
  const swipeGesture = async (shouldEnable: boolean, menu?: string | null): Promise<HTMLGascoMenuElement | undefined> => {
    const menuEl = await get(menu);
    if (menuEl) {
  ...
  const get = async (menu?: string | null): Promise<HTMLGascoMenuElement | undefined> => {
   ...
   */
  const getOpen = async (): Promise<HTMLGascoMenuElement | undefined> => {
    await waitUntilReady();
    return _getOpenSync();
  };
   ...
   */
  const getMenus = async (): Promise<HTMLGascoMenuElement[]> => {
    await waitUntilReady();
    return getMenusSync();
  };
  ...
  const _getOpenSync = (): HTMLGascoMenuElement | undefined => {
    return find((m) => m._isOpen);
  };

  const getMenusSync = (): HTMLGascoMenuElement[] => {
  ...
  const find = (predicate: (menu: MenuI) => boolean): HTMLGascoMenuElement | undefined => {
  ...
  const waitUntilReady = () => {
    return Promise.all(
      Array.from(document.querySelectorAll('gasco-menu')).map(
  ...
  if (typeof document !== 'undefined') {
    document.addEventListener('gascoBackButton', (ev: any) => {
