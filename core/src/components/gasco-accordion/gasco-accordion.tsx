...
@Component({
  tag: 'gasco-accordion',
  styleUrl: 'gasco-accordion.md.scss',
  shadow: {
    delegatesFocus: true,
  },
})
export class GascoAccordion implements ComponentInterface {
  private accordionGroupEl?: HTMLGascoAccordionGroupElement | null;
   ...
   * value.
   */
  @Prop() value = `gasco-accordion-${accordionIds++}`;
  ...
  /**
   * The slot inside of `gasco-item` to
   * place the toggle icon. Defaults to `'end'`.
   */
  @Prop() toggleIconSlot: 'start' | 'end' = 'end';

  connectedCallback() {
    const accordionGroupEl = (this.accordionGroupEl = this.el?.closest('gasco-accordion-group'));
    if (accordionGroupEl) {
      this.updateState(true);
      addEventListener(accordionGroupEl, 'gascoChange', this.updateListener);
    }
  }
  ...
  disconnectedCallback() {
    ...
    if (accordionGroupEl) {
      removeEventListener(accordionGroupEl, 'gascoChange', this.updateListener);
    }
  }
  ...
  componentDidLoad() {
     ...
     * just set gascoItem.button = true and
  ...
  private setItemDefaults = () => {
     ...
     */
    gascoItem.button = true;
    gascoItem.detail = false;
     ...
     */
    if (gascoItem.lines === undefined) {
      gascoItem.lines = 'full';
    }
  };
  ...
  private getSlottedHeaderIonItem = () => {
    ...
    const gascoItem =
      ...
      (slot.assignedElements().find((el) => el.tagName === 'GASCO-ITEM') as HTMLGascoItemElement | undefined);
  ...
  private setAria = (expanded = false) => {
     ...
     * gasco-item because that is what will be focused
     */
    const root = getElementRoot(gascoItem);
  ...
  private slotToggleIcon = () => {
     ...
     */
    const existingToggleIcon = gascoItem.querySelector('.gasco-accordion-toggle-icon');
  ...
  private getNextSibling = () => {
    ...
    if (nextSibling?.tagName !== 'GASCO-ACCORDION') {
      return;
    }

    return nextSibling as HTMLGascoAccordionElement;
  };
  ...
  private getPreviousSibling = () => {
    ...
    if (previousSibling?.tagName !== 'GASCO-ACCORDION') {
      return;
    }

    return previousSibling as HTMLGascoAccordionElement;
  };
