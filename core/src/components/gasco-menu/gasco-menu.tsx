...
@Component({
  tag: 'gasco-menu',
  styleUrl: 'gasco-menu.md.scss',
  shadow: true,
})
export class GascoMenu implements ComponentInterface, MenuI {
  ...
  private handleFocus = (ev: FocusEvent) => {
     ...
     * open does not contain this gasco-menu, then gasco-menu's
  ...
  @Element() el!: HTMLGascoMenuElement;
   ...
   */
  @Event() gascoWillOpen!: EventEmitter<void>;

  /**
   * Emitted when the menu is about to be closed.
   */
  @Event() gascoWillClose!: EventEmitter<void>;
  /**
   * Emitted when the menu is open.
   */
  @Event() gascoDidOpen!: EventEmitter<void>;

  /**
   * Emitted when the menu is closed.
   */
  @Event() gascoDidClose!: EventEmitter<void>;

  /**
   * Emitted when the menu state is changed.
   * @internal
   */
  @Event() protected gascoMenuChange!: EventEmitter<MenuChangeEventDetail>;

  async connectedCallback() {
    // TODO: connectedCallback is fired in CE build
    // before WC is defined. This needs to be fixed in Stencil.
    if (typeof (customElements as any) !== 'undefined') {
      await customElements.whenDefined('gasco-menu');
    }
    ...
    if (this.el.contains(content)) {
      console.error(
        `Menu: "contentId" should refer to the main view's gasco-content, not the gasco-content inside of the gasco-menu.`
      );
    }
  ...
  async componentDidLoad() {
    this.gascoMenuChange.emit({ disabled: this.disabled, open: this._isOpen });
    this.updateState();
  }
  ...
  @Listen('gascoSplitPaneVisible', { target: 'body' })
  ...
  private async startAnimation(shouldOpen: boolean, animated: boolean): Promise<void> {
    ...
    const easing = mdEasing
    const easingReverse =  mdEasingReverse;
  ...
  private canStart(detail: GestureDetail): boolean {
    ...
    const isModalPresented = !!document.querySelector('gasco-modal.show-modal');
  ...
  private beforeAnimation(shouldOpen: boolean) {
    ...
    if (shouldOpen) {
      this.gascoWillOpen.emit();
    } else {
      this.gascoWillClose.emit();
    }
  }
  ...
  private afterAnimation(isOpen: boolean) {
    ...
    if (isOpen) {
       ...
       */
      const focusedMenu = document.activeElement?.closest('gasco-menu');
