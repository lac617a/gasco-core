...
@Component({
  ...
  styleUrl: 'gasco-accordion-group.md.scss',
  shadow: true,
})
export class GascoAccordionGroup implements ComponentInterface {
  @Element() el!: HTMLGascoAccordionGroupElement;
  ...
  async onKeydown(ev: KeyboardEvent) {
    ...
    const accordionEl =
      activeElement.tagName === 'GASCO-ACCORDION' ? activeElement : activeElement.closest('gasco-accordion');
    if (!accordionEl) {
      return;
    }

    const closestGroup = accordionEl.closest('gasco-accordion-group');
    ...
    let accordion: HTMLGascoAccordionElement | undefined;
    if (ev.key === 'ArrowDown') {
  ...
  private findNextAccordion(accordions: HTMLGascoAccordionElement[], startingIndex: number) {
  ...
  private findPreviousAccordion(accordions: HTMLGascoAccordionElement[], startingIndex: number) {
  ...
  @Method()
  async getAccordions() {
    return Array.from(this.el.querySelectorAll(':scope > gasco-accordion')) as HTMLGascoAccordionElement[];
  }
